...
import React, { useState, Dispatch, SetStateAction, useEffect, useCallback, ReactNode } from 'react';
...
function pdfswap(x: number, x2: number, y: ReactNode, z: ReactNode){
       if(x == x2){
              return z
       }
       else{
...
export default function Writing(Timer: number, setPage: pagesetter, mouse: point, extravars: reactvar[]) {
    ...
    return <div key = "pagewrapper" className = "pagewrapper">
            <svg className="animsvg" fill = "true"
              ...
              const imageheight = 0.6*tabdims.y
